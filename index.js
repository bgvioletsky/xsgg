// const xbsTools = require('./xbsTools');
const fs = require('fs');
...
const xbsTools = {
    ...
    async downloadFile(urlString, destPath) {
        ...
        const downloader = protocol === 'https:' ? https : http;

        const response = await new Promise((resolve, reject) => {
            downloader.get(urlString, resolve).on('error', reject);
        });
    ...
    async convertXbsToJson(xbsFilePath, jsonFilePath,isWrite) {
        try {
            let data;
            if (xbsFilePath.startsWith('http')) {
                const tempFilePath = './tempfile.xbs';
                await this.downloadFile(xbsFilePath, tempFilePath);
                data = fs.readFileSync(tempFilePath);
                fs.unlinkSync(tempFilePath); // 删除临时文件
            } else {
                data = fs.readFileSync(xbsFilePath);
            }

            const json = this.XBS2Json(data);
            if(isWrite){
            ...
            console.error("将XBS转换为JSON时出错：", error);
            throw new Error("将XBS转换为JSON时出错");
        }
    },
    async convertJsonToXbs(jsonFilePath, xbsFilePath,isWrite) {
        try {
            let data;
            if (jsonFilePath.startsWith('http')) {
            ...
            const xbs = this.Json2XBS(data);
            if(isWrite){
            fs.writeFileSync(xbsFilePath, xbs);
            return xbs;}
        } catch (error) {
            console.error("将XBS转换为JSON时出错：", error);
            throw new Error("将XBS转换为JSON时出错");
